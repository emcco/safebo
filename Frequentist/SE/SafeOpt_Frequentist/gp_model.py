...
def plot_true(true_x, true_y, jmin):
    ...
    ax.text(max_x, plt.ylim()[0] - 0.05 * (plt.ylim()[1] - plt.ylim()[0]), f'{max_x:.2f}', color='b', fontsize=10, ha='center')
    ax.text(-0.25, max_y, f'{max_y:.2f}', color='b', fontsize=10, ha='center')
...
def construct_uncertainty_width(model, x, R, delta, lambd):
    ...
    K_inv_knx = Kn_inv @ knx
    term1 = R * torch.norm(K_inv_knx, dim=0)
    ...
    return etaN, term1, term2 

def plot_eta(json_filename):
    # Load data
    with open(json_filename, "r") as json_file:
        data = json.load(json_file)

    iterations = list(range(1, len(data["eta_x"]) + 1))

    fig, (ax1, ax2) = plt.subplots(2, 1, sharex=True, figsize=(8, 6), gridspec_kw={'height_ratios': [2, 1]})
    plt.subplots_adjust(left=0.08, right=0.99, top=0.95, bottom=0.1)

    # Define the break range (modify as needed)
    break_point = 0.75  # Values below this go in ax2, above in ax1

    # Plot for large values
    ax1.plot(iterations, data["eta_x"], label=r"$\eta_x$", marker="o", linestyle="-")
    ax1.plot(iterations, data["term1"], label="Term a", marker="s", linestyle="--")
    ax1.plot(iterations, data["term2"], label="Term b", marker="d", linestyle="-.")
    
    # Plot for small values
    ax2.plot(iterations, data["eta_x"], marker="o", linestyle="-")
    ax2.plot(iterations, data["term1"], marker="s", linestyle="--")
    ax2.plot(iterations, data["term2"], marker="d", linestyle="-.")

    # Set y-limits
    ax1.set_ylim(break_point + 0.1, max(max(data["eta_x"]), max(data["term1"]), max(data["term2"])) + 0.5)
    ax2.set_ylim(min(min(data["eta_x"]), min(data["term1"]), min(data["term2"])) - 0.1, break_point - 0.1)

    # Remove space between plots
    plt.subplots_adjust(hspace=0.05)

    # Add break markers (diagonal lines)
    # d = 0.015
    ...
    # fig.text(0.02, 0.5, "Values", va='center', rotation='vertical')
    # ax1.set_title("Uncertainty Terms over Iterations")
    ax1.legend()
    ax1.grid(True, which='both', linestyle='--', linewidth=0.5) 
    ax2.grid(True, which='both', linestyle='--', linewidth=0.5) 
    string = 'comp' + globals.comp + '/comp' + globals.comp + '-' + str(globals.index) + '/plot' + "_" + 'etaX' + ".png"
    fig.savefig(string, format='png')
    plt.show()
...
def get_bounds_RKHS(model, x, B=globals.B, R=globals.R, delta=globals.delta, lambd=globals.lambd):
    ...
    eta_x = construct_uncertainty_width(model, x, R, delta, lambd)[0]
