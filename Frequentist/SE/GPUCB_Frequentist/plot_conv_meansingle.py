...
for file in files:
    with open(file, 'r') as f:
        ...
        for i in range(len(mean_values)):
            # conv.append(mean_values[i] / y_max)
...
for file in files:
    with open(file, 'r') as f:
        ...
        ax.plot(iterations, conv, alpha=0.1)

# Plot mean
ax.plot(iterations, mean_values_mean, linewidth=2, color='black', marker='*')
...
ax.set_xlim(1,20)
ax.set_ylabel('Regret R(n)', fontsize=12)
ax.legend()
# ax.set_title(f'Convergence Plot: GP-UCB - Frequentist view (n={len(files)})')
ax.grid(True, alpha=0.2)

timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
string = f'plot_mean_convergence_'+ globals.comp +'.png'
