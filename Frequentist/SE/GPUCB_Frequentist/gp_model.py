...
def plot(model, true_x, true_y, test_x, jmin, b, x_sample, type, y_target, warning):
    ...
    # Added data point
    if not type == 'initial':
        ax.plot(x_sample.detach().numpy(), y_target.detach().numpy(), 'r*')
    ...
    # Split into contiguous segments if needed
        ...
        for i, segment in enumerate(x_segments):
            ...
            ax.hlines(
                y=-xlim,                              # The y-value where the line will appear
                xmin=segment[0], 
                xmax=segment[-1],
                colors='green',
    ...
    mean, lower, upper = get_bounds_RKHS(model, safe_x)
    x_ucb = safe_x[torch.argmax(upper).item()].item()
    ax.vlines(x_ucb, -xlim, xlim, color='red', linestyles='--')
    # ax.plot([], [], '', label=warning)
    ax.set_xlabel('Input x', fontsize=12)
    ax.set_ylabel('Output y', fontsize=12)
    # ax.set_title('SafeOpt - Bayesi', fontsize=18, fontweight='bold')
