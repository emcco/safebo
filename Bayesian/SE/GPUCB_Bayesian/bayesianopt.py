...
class BayesianOpt:
    ...
    def optimize(self, model, Lk, Lmean):

        mean, lower, upper = gpm.get_mean_bounds(model, self.test_x, Lk, Lmean)
        safe_set= so.classify_safe_set(model, self.test_x, self.jmin, Lk, Lmean)
        lower_max = torch.max(lower)
        max_set = so.classify_maximizer(model, self.test_x, Lk, Lmean)
        ...
        lower_safe = gpm.get_mean_bounds(model, x_exp_safe, Lk, Lmean)[1]
        lower_unsafe = gpm.get_mean_bounds(model, x_exp_unsafe, Lk, Lmean)[1]
        ...
        optimizer = modsafeopt.ModSafeOpt(x0_max, x0_exp, self.test_x, model, self.jmin, flag, Lk, Lmean)
