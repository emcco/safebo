...
def plot(model, Lk, Lmean, true_x, true_y, test_x, jmin, b, x_sample, type, y_target, warning):
    ...
    # ax.set_title('SafeOpt - Bayesian View', fontsize=18, fontweight='bold')
    ax.legend()
...
def comp_post_mean_lip(model, Lk, train_x, train_y):
    ...
    lederer = K_inv @ train_y
    weight_norm_old = torch.linalg.norm(lederer)
...
def get_mean_bounds(model, test_x, Lk, Lmean):
    ...
    indices = torch.nonzero(lower > -0.5, as_tuple=True)[0]
