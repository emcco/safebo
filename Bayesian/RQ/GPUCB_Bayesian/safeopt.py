...
def classify_safe_set(model, test_x, jmin, Lk, Lmean):
    safe_set = torch.zeros(len(test_x), dtype=torch.bool)
    if torch.any(safe_set):
        print('ERROR: Not a single safe x found?????')
        print(safe_set)

    for i, x in enumerate(test_x):
        lower = gpm.get_mean_bounds(model, x.unsqueeze(0), Lk, Lmean)[1]
...
# def classify_expander(model, test_x, jmin, Lk, Lmean):
    mean, lower, upper = gpm.get_mean_bounds(model, test_x)
    safe_set = classify_safe_set(model, test_x, jmin, Lk, Lmean)
    ...
    for i in range(upper.size(0)):
        ...
        lower_add = gpm.get_mean_bounds(model_add, test_x)
        safe_set_add = classify_safe_set(model, test_x, jmin, Lk, Lmean)
...
def get_expander_ic(test_x, safe_set, model):
    # Find all pairs where one value is safe and the next is unsafe, or vice versa
    # pairs = []
    # for i in range(len(safe_set) - 1):
    #     if safe_set[i] != safe_set[i + 1]:  # Check for adjacent differing values
    #         if safe_set[i]:
    #             pairs.append(((i, test_x[i]), (i+1, test_x[i + 1])))
    #         else:
    #             pairs.append(((i+1, test_x[i+1]), (i, test_x[i])))
    ...
    # x_safe = chosen_pair[0][1].item()
    # x_unsafe = chosen_pair[1][1].item()

    # x_exp = torch.tensor([[[x_safe, x_unsafe]]])
    pairs = torch.empty((0, 4))
...
def classify_maximizer_old(upper, lower_max):

    max_set = upper >= lower_max
    
    return max_set

def classify_maximizer(model, test_x, Lk, Lmean):

    lower = gpm.get_mean_bounds(model, test_x, Lk, Lmean)[1]
    lower_max = torch.max(lower)

    max_set = lower >= lower_max

    for i, x in enumerate(test_x):
        upper = gpm.get_mean_bounds(model, x.unsqueeze(0), Lk, Lmean)[2]
