...
num_latest = globals.iterations

iterations = np.arange(1, num_latest)
...
# ax.set_title(f'Convergence Plot: SafeOpt - Bayesian View (n={len(files)})')
ax.grid(True, alpha=0.2)
ax.set_xlim(1, globals.iterations + 1)

# Find iteration where mean convergence function reaches or falls below 0.003
convergence_iteration = None
for i, val in enumerate(mean_values_mean):
    if val <= 3*globals.noise_stddev.item():
...
string = 'plot_conv_RQ_Bayesian.png'
