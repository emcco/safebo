...
num_latest = globals.iterations - 1 
max_it = 35
...
ax.fill_between(
    iterations,
    mean_values_mean - 2 * mean_values_stddev,
    mean_values_mean + 2 * mean_values_stddev,
    color='gray',
    alpha=0.4,
    label=r'$\text{Confidence Region}$ $\mu \pm 2 \cdot \sigma$'
)
...
ax.set_ylabel('Regret R(n)', fontsize=18)
...
if convergence_iteration is not None:
    ax.axhline(conv_crit, color='red', linestyle='--', label=f'Converged at iteration {convergence_iteration}')
else:
    ax.axhline(conv_crit, color='red', linestyle='--')

ax.text(0.5, conv_crit-0.03, r'$h_{opt}$', 
